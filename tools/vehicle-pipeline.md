...
- `car-dirt.ts` ведёт реальные Vehicle (`gt_vaz2110`) и проверяет: скорость запыления по покрытиям (песок полностью за 3–10 км, гравий медленнее, асфальт — медленная дорожная плёнка 60–300 км), независимость одинаковых моделей, мойку припаркованной машины, царапины от лобового удара, save round-trip dirt/scratches, миграцию старого сейва (в том числе с полями `dents`/`damage` удалённых систем повреждений) и отсутствие царапин от лёгкого касания и падения на колёса.
