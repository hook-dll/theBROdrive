...
// Behind the wheel nothing but the exit saves: a save booked by the crash itself would
// capture the fatal one, and the reload would put the player back into it. Dust and
// the lamps ride along with the exit.
world.apply({ t: 'enter_car', carId: 'car:test' });
await Promise.resolve();
check('re-entering autosaves', calls.length === 5, `${calls.length} writes`);
world.apply({ t: 'car_body_condition', carId: 'car:test', dirt: 0.42, scratches: 0.17 });
world.apply({ t: 'car_lights', carId: 'car:test', headlightMode: 'low', taillightsOn: true, reverseLightsOn: false });
world.apply({ t: 'inventory', items: [], selected: 0 });
await Promise.resolve();
check('nothing at the wheel saves', calls.length === 5, `${calls.length} writes`);
world.apply({ t: 'exit_car' });
await Promise.resolve();
check('the exit saves', calls.length === 6, `${calls.length} writes`);
const exited = calls[5]?.state.cars['car:test'];
check(
  'the exit carries dust and lamps',
  exited?.dirt === 0.42 && exited.scratches === 0.17 && exited.headlightMode === 'low',
  `dirt ${exited?.dirt}, scratches ${exited?.scratches}, lamps ${exited?.headlightMode}`,
);
const bodyRoundTrip = decodeSaveCode(encodeSaveCode(calls[5]!.state)).cars['car:test'];
check(
  'save code keeps dust',
  bodyRoundTrip?.dirt === 0.42 && bodyRoundTrip.scratches === 0.17,
  `dirt ${bodyRoundTrip?.dirt}, scratches ${bodyRoundTrip?.scratches}`,
);
...
world.apply({ t: 'enter_car', carId: 'car:test' });
await Promise.resolve();
check('unsubscribe stops autosaves', calls.length === 6, `${calls.length} writes`);
