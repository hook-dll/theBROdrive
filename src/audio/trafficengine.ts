...
export class TrafficEngineAudio {
  ...
  constructor(private readonly mixer: AudioMixer) {
    ...
    this.engineGain.gain.value = 0.48;
  ...
  update(state: VehicleAudioState, x: number, y: number, z: number): void {
    ...
    this.lowpass.frequency.setTargetAtTime(
      700 + 4500 * (0.35 * rev + 0.65 * load),
      now,
      0.06,
    );
    const engineLevel = state.engineRunning ? 0.22 + 0.14 * (0.45 * rev + 0.55 * load) : 0;
    ramp(this.out.gain, engineLevel, now, 0.12);
  }
